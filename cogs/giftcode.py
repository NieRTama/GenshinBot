...
GAMEWITH_URL = "https://gamewith.jp/genshin/article/show/231856"
GIFT_BASE_URL = "https://genshin.hoyoverse.com/ja/gift?code="
...
CODE_PATTERN = re.compile(r'\b([A-Z][A-Z0-9]{9,19})\b')
EXCLUDE_CODES = {
    "JAVASCRIPT", "STYLESHEET", "GOOGLETAGMANAGER",
    "CLOUDFLARE", "ADDEVENTLISTENER",
}
...
def _fetch_codes() -> list:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "gift?code=" in href:
            code = href.split("code=")[-1].strip().upper()
            if code:
                codes.add(code)

    # code / strong / b / td / span タグ内のコード形式テキスト
    for tag in soup.find_all(["code", "strong", "b", "td", "span"]):
        text = tag.get_text(strip=True).upper()
        if CODE_PATTERN.fullmatch(text) and text not in EXCLUDE_CODES:
            codes.add(text)
...
class GiftCodeCog(commands.Cog):
    ...
    async def giftcode_check(self):
        ...
        for code in new_codes:
            ...
            embed.set_footer(text="GameWithから自動検出 | GenshinBot")
